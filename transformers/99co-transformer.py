...
def update_mrt(df, db: MotherDuckDBConnector):
...
def update_hawker(df, db: MotherDuckDBConnector):
...
def update_supermarket(df, db: MotherDuckDBConnector):
...
def update_primary_school(df, db: MotherDuckDBConnector):
...
def update_mall(df, db: MotherDuckDBConnector):
...
def change_data_capture(df, db: MotherDuckDBConnector) -> None:
    """ 
    changed:  
...
def transform(db: MotherDuckDBConnector, date: str = None):
    if date is None:
        date = datetime.now().strftime('%Y-%m-%d')

    df = read_df_from_s3(
        f"rental_prices/ninety_nine/processed_v2/{date}.parquet.gzip")
    ...
    df = augment_df_w_add_info(df, db)
